...
def create_graph(minuts, datarow):
    ...
    # (id, temperature, humidity, earthhumidity, timestamp)
    # datarow should be either 1, 2, or 3 (NOT 0 or 4)
    values = [row[datarow] for row in recent_data]  # Earth humidity
    
    # Enum
    # A way to convert between numbers and names
    ...
    ax.set_title(f"{datanames.get(datarow)} (Last {minuts} Minutes)")
    ax.set_ylabel("Value")
    ax.set_xlabel("Time")
    ax.grid(True)
...
def index():
    ...
    <html>
        ...
        <body>
            ...
            <h1>Live Graph</h1>
            <img id="graph" src="" alt="Graph will appear here... ETA: 5 seconds" style="width:1200px; border:3px solid #ccc;"/>
            ...
            <script>
                ...
                // Update graph
                socket.on('earth_graph', function(data) {
...
def sensor_loop():
    while True:        
        ...
        # create_graph(Minuttes, Datarow)
        # (id, temperature, humidity, earthhumidity, timestamp)
        # Earth Graph
        graph_img = create_graph(15,3)
        if graph_img:
            socketio.emit('earth_graph', {'image': graph_img})

        # Temp Graph
        graph_img = create_graph(15,1)
        if graph_img:
            socketio.emit('temp_graph', {'image': graph_img})

        # Air Graph
        graph_img = create_graph(15,2)
        if graph_img:
            socketio.emit('air_graph', {'image': graph_img})
        print(f"Sleeping for 1 minut")
